...
2. Both sides valid semver **and** `major` is the same **and** that shared major is `0` → this is the `0.y.z` case, where semver gives no stability guarantee at all and, by convention (and how npm itself treats it), the **minor** is the breaking-change slot a `1.y`+ package would use its major for:
   ...
   - Different minor (`0.y` → `0.y'`) → needs verification, exactly like a major jump (see grouping note below). `@vercel/ncc` (`^0.38.0`, the build tool several of the actions under `actions/` depend on) is exactly this shape — `0.38.0` → `0.39.0` is a potentially-breaking jump, not a safe patch, even though the leading digit never changed.
...
This repo ships compiled Node actions under `actions/` (moved here from `VirtoCommerce/vc-github-actions` in VCST-5414) — each `actions/<name>/action.yml` runs `dist/index.js`, the bundled output, not `src/`. A dependency bump only changes what actually runs once `dist/` is rebuilt to match. Two workflows enforce this:

- **`.github/workflows/check-dist.yml`** (`detect` / `build-and-diff` checks) — rebuilds `dist/` from the PR's `src/` + lockfile and fails if it doesn't byte-match what's committed. Only paths under `actions/` trigger it; Dependabot npm PRs here are always under `/actions/<name>`.
